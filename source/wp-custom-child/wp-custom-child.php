...
function wctg_custom_child_theme_process() {
	...
	if(isset($_POST['custom-child-create']) && wp_verify_nonce( $_POST['wp-easy-nonce'], 'custom-child-theme-creation' ) ){ 
			...
			if($result !== TRUE) {
				...
				wp_redirect( $go_url );
			} else if( !isset( $_POST['parent-screenshot'] ) && !in_array( $_FILES['fileUpload']['type'], array('image/jpeg', 'image/png', 'image/jpg') ) ){
				$go_url = $go_url = add_query_arg( array(
					'msg'=> __('Invalid file type. Please upload jpeg, jpg or png file.','wp-child-theme-generator'),
					'error_type' => 'empty'
